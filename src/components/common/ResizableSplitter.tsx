...
export const ResizableSplitter: React.FC<ResizableSplitterProps> = ({
  ...
  return (
    <div
      className={`relative flex-shrink-0 ${isCollapsed ? 'w-0' : 'w-2'} flex flex-col justify-center items-center group z-10 cursor-col-resize select-none bg-transparent ${className}`}
    ...
    >
      ...
      {onCollapse && (
        ...
        >
          {isCollapsed ? <Icons.LeftArrow /> : <Icons.RightArrow />}
        </button>
      )}
    </div>
  );
